...
\todo{1x1 literatur seite 100 lesen und hinzufügen}
...
\begin{itemize}
    \item \textbf{Bauchsignal (\texttt{abdomen}) ($32 \si{\hertz}$)}: \todo{describe}
    ...
    \item \textbf{Drucksignal (\texttt{flow}) ($256 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Bewegungssignal (\texttt{movement}) ($32 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Sauerstoffsättigung (\texttt{pleth}) ($128 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Pulssignal (\texttt{pulse}) ($32 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Schnarchmikrofonsignal (\texttt{schnarch}) ($256 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Sauerstoffgehalt (\texttt{spo2}) ($32 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Brustkorbsignal (\texttt{thorax}) ($32 \si{\hertz}$)}: \todo{describe}
    \item \textbf{Brustkorb \& Bauchsignal (\texttt{thoraxabdomen}) ($32 \si{\hertz}$)}: \todo{describe}
    \item \textbf{EDF Informationen (\texttt{edfAnnotations}) ($60 \si{\hertz}$)}: \todo{describe}
\end{itemize}
